...
namespace KDL{
    ...
    RigidBodyInertia::RigidBodyInertia(double m_, const Vector& c_, const RotationalInertia& Ic):
        m(m_),h(m*c_){
        //I=Ic-c x c x
        Vector3d c_eig=Map<Vector3d>((double*)c_.data);
        Map<Matrix3d>(I.data)=Map<Matrix3d>((double*)Ic.data)-m_*(c_eig*c_eig.transpose()-c_eig.dot(c_eig)*Matrix3d::Identity());
    }
    ...
    RigidBodyInertia operator*(const Frame& T,const RigidBodyInertia& I){
        ...
        Vector3d h_eig = Map<Vector3d>((double*)I.h.data);
        ...
        Map<Matrix3d>(Ib.data) = R*((Map<Matrix3d>((double*)I.I.data)+rcrosshcross+hmrcrossrcross)*R.transpose());
    ...
    RigidBodyInertia operator*(const Rotation& M,const RigidBodyInertia& I){
        ...
        //hb=R*h
        //Ib = R(Ia)R' with r=0
        Matrix3d R = Map<Matrix3d>((double*)M.data);
        RotationalInertia Ib;
        Map<Matrix3d>(Ib.data) = R.transpose()*(Map<Matrix3d>((double*)I.I.data)*R);
    ...
    RigidBodyInertia RigidBodyInertia::RefPoint(const Vector& p){
        ...
        Vector3d r_eig = Map<Vector3d>((double*)p.data);

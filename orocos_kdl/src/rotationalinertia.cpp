...
namespace KDL
{
	...
	Vector RotationalInertia::operator*(const Vector& omega) const {
		// Complexity : 9M+6A
        Vector result;
        Map<Vector3d>(result.data)=Map<Matrix3d>((double*)this->data)*Map<Vector3d>((double*)omega.data);
        return result;
 	}

    RotationalInertia operator*(double a, const RotationalInertia& I){
        RotationalInertia result;
        Map<Matrix3d>(result.data)=a*Map<Matrix3d>((double*)I.data);
        return result;
    }
    
    RotationalInertia operator+(const RotationalInertia& Ia, const RotationalInertia& Ib){
        RotationalInertia result;
        Map<Matrix3d>(result.data)=Map<Matrix3d>((double*)Ia.data)+Map<Matrix3d>((double*)Ib.data);
        return result;
    }
}

...
#include "chainiksolverpos_lma_jl.hpp"
#include <iostream>
...
ChainIkSolverPos_LMA_JL::ChainIkSolverPos_LMA_JL(
...
void ChainIkSolverPos_LMA_JL::compute_fwdpos(const VectorXq& q) {
	using namespace KDL;
...
void ChainIkSolverPos_LMA_JL::compute_jacobian(const VectorXq& q) {
	using namespace KDL;
	unsigned int jointndx=0;
	for (unsigned int i=0;i<chain.getNrOfSegments();i++) {
		const Segment& segment = chain.getSegment(i);
...
void ChainIkSolverPos_LMA_JL::display_jac(const KDL::JntArray& jval) {
	VectorXq q;
...
int ChainIkSolverPos_LMA_JL::CartToJnt(const KDL::JntArray& q_init, const KDL::Frame& T_base_goal, KDL::JntArray& q_out) {
	using namespace KDL;
	double v      = 2;
	double tau    = 10;
	double rho;

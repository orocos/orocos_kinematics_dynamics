...
int TreeJntToJacSolver::JntToJac(const JntArray& q_in,Jacobian& jac,const std::string& segmentname){
	...
	if (q_in.rows()!=tree.getNrOfJoints()||jac.rows()!=tree.getNrOfJoints())
		return -1;
	...
	while(it!=root){
		...
		Twist t_local=Twist::Zero();
		if(it->second.segment.getJoint().getType()!=Joint::None)
			t_local=it->second.segment.twist(q_in(q_nr),0.0);
		...
		t_local.RefPoint(T_total.p-T_local.p);

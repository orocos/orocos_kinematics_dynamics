...
#include <kdl/kdltk/motionproperties.hpp>
...
namespace KDL 
{
        ...
        : public TemplateTypeInfo<JointType,true>
    {
        ...
        virtual bool composeTypeImpl(const PropertyBag& source, JointType& result) const {
            return composeProperty( source, result );
        }
    };

    template<class TransmissionType>
    struct TransmissionTypeInfo
        : public TemplateTypeInfo<TransmissionType,false>
    {
        TransmissionTypeInfo(std::string name) : TemplateTypeInfo<TransmissionType,false>(name) {}

        virtual bool decomposeTypeImpl(const TransmissionType& source, PropertyBag& targetbag ) const {
            decomposeProperty( source, targetbag );
            return true;
        }
        
        virtual bool composeTypeImpl(const PropertyBag& source, TransmissionType& result) const {
            return composeProperty( source, result );
        }
    };

    template<class KinematicFamilyType>
    struct KinematicFamilyTypeInfo
        : public TemplateTypeInfo<KinematicFamilyType,false>
    {
        KinematicFamilyTypeInfo(std::string name) : TemplateTypeInfo<KinematicFamilyType,false>(name) {}

        virtual bool decomposeTypeImpl(const KinematicFamilyType& source, PropertyBag& targetbag ) const {
            return decomposeProperty( source, targetbag );
        }
        
        virtual bool composeTypeImpl(const PropertyBag& source, KinematicFamilyType& result) const {
            return composeProperty( source, result );
        }
    };
...
//         AlwaysAssignChecker<Vector>,true >("vector") );
        ...
        ti->addType( new JointTypeInfo<JointTransX>("JointTransX") );
        ti->addType( new JointTypeInfo<JointTransY>("JointTransY") );
        ti->addType( new JointTypeInfo<JointTransZ>("JointTransZ") );
        ti->addType( new JointTypeInfo<JointRotX>("JointRotX") );
        ti->addType( new JointTypeInfo<JointRotY>("JointRotY") );
        ti->addType( new JointTypeInfo<JointRotZ>("JointRotZ") );


        ti->addType( new TransmissionTypeInfo<UnitTransmission>("UnitTransmission"));
        ti->addType( new TransmissionTypeInfo<LinearTransmission>("LinearTransmission"));
        ti->addType( new TransmissionTypeInfo<Kuka361Transmission>("Kuka361Transmission"));

        ti->addType( new KinematicFamilyTypeInfo<SerialChain>("SerialChain"));
        ti->addType( new KinematicFamilyTypeInfo<ZXXZXZ>("ZXXZXZ"));
